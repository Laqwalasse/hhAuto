...
// Очистка процессов Google Chrome перед запуском скрипта. Если есть активные процессы, то он выдаст ошибку.

const killAllChromeProcesses = () => {
    return new Promise((resolve, reject) => {
        exec('taskkill /F /IM chrome.exe /T', (error, stdout, stderr) => {
            if (stderr && stderr.includes('не найден процесс "chrome.exe".')) {
...
killAllChromeProcesses()
    ...
    .catch(error => {
        if (error && error.message && error.message.includes('не найден процесс "chrome.exe".')) {
